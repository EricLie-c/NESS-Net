...
```bash
python train.py --output_dir ./checkpoints --general_model ./checkpoints/NESS_general_35.pth
```
...
We have already provided NAMLab-generated edge maps for 9 datasets (in folders suffixed with `-lab`). However, if you intend to process additional datasets, please follow the configuration steps below.

Replace the `run_dataset.cpp` in the original project  [NAMLab](https://github.com/YunpingZheng/NAMLab) with the `run_dataset.cpp` we provided. 

Afterward, configure the code and dependencies according to the original project's instructions.

Then, run the program and follow the on-screen prompts to enter the input dataset path and output directory. 
...
![Quantitative Comparison](./figure/quantitative_comparison.png)

### Qualitative Comparison

![Qualitative Comparison](./figure/qualitative_comparison.png)