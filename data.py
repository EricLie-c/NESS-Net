...
class SalObjDataset(data.Dataset):
    def __init__(
            self, image_root, depth_root, gt_root, mask_root, gray_root, rgb_edge_root, depth_edge_root, trainsize, general_stage=True
    ):
        ...
        self.rgb_edges = [os.path.join(rgb_edge_root, f) for f in os.listdir(rgb_edge_root) if f.endswith('.png')]
        self.depth_edges = [os.path.join(depth_edge_root, f) for f in os.listdir(depth_edge_root) if f.endswith('.png')]
        ...
        self.rgb_edges = sorted(self.rgb_edges)
        self.depth_edges = sorted(self.depth_edges)
    ...
    def __getitem__(self, index):
        # prepare inputs for general model or student model
        ...
        rgb_edge = self.binary_loader(self.rgb_edges[index])
        depth_edge = self.binary_loader(self.depth_edges[index])

        if not self.general_stage:
        ...
        rgb_edge = self.resize_transform(rgb_edge)
        depth_edge = self.resize_transform(depth_edge)
        rgb_edge = self.to_tensor_transform(np.array(rgb_edge).astype(np.uint8))
        depth_edge = self.to_tensor_transform(np.array(depth_edge).astype(np.uint8))
        ...
        if not self.general_stage:
            ...
            # )
            depth_edge_teacher = depth_edge
            depth_teacher = self.to_tensor_transform(self.resize_transform(depth_teacher))

            return image, depth, gt, mask, gray, rgb_edge, depth_edge, \
    ...
    def filter_files(self):
        ...
        for img_path, depth_path, gt_path, mask_path, gray_path, rgb_edge_path, depth_edge_path in zip(
                self.images, self.depths, self.gts, self.masks, self.grays, self.rgb_edges, self.depth_edges
        ):
            ...
            rgb_edge = Image.open(rgb_edge_path)
            ...
            if img.size == gt.size:
                ...
                rgb_edges.append(rgb_edge_path)
                depth_edges.append(depth_edge_path)
...
def get_loader(
        image_root, depth_root, gt_root, mask_root, gray_root, rgb_edge_root, depth_edge_root, batchsize, trainsize,
        shuffle=True, num_workers=12, pin_memory=True, general_stage=True
):
    dataset = SalObjDataset(
        image_root, depth_root, gt_root, mask_root, gray_root, rgb_edge_root, depth_edge_root, trainsize, general_stage
    )
...
class test_dataset:
    def __init__(self, image_root, depth_root, rgb_edge_root, depth_edge_root, testsize):
        ...
        self.rgb_edges = [
            os.path.join(rgb_edge_root, f) for f in os.listdir(rgb_edge_root) if f.endswith('.bmp') or f.endswith('.png')
        ]
        self.depth_edges = [
            os.path.join(depth_edge_root, f) for f in os.listdir(depth_edge_root) if f.endswith('.bmp') or f.endswith('.png')
        ]
        self.images = sorted(self.images)
        self.depths = sorted(self.depths)
        self.rgb_edges = sorted(self.rgb_edges)
    ...
    def load_data(self):
        ...
        rgb_edge = self.binary_loader(self.rgb_edges[self.index])
        depth_edge = self.binary_loader(self.depth_edges[self.index])
        ...
        image_edge = self.to_tensor_transform(np.array(self.resize_transform(rgb_edge)).astype(np.uint8)).unsqueeze(0)
        image = self.resize_transform(image)
        ...
        depth_edge = self.to_tensor_transform(np.array(self.resize_transform(depth_edge)).astype(np.uint8)).unsqueeze(0)
        depth = self.resize_transform(depth)
